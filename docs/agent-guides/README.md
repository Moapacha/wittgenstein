...
| [`image-port.md`](image-port.md)                   | M1A   | image contributor / agent  | landed image port reference, Brief H practices, F1 + F2 amendments    |
...
- **You need to audit or extend the landed image line:** `image-port.md` is the canonical M1A reference; `image-to-audio-port.md` is supporting cross-line context; `docs/research/briefs/H_codec_engineering_prior_art.md` is the implementation checklist; `docs/research/briefs/G_image_network_clues.md` is decoder rationale; `Issue #70` tracks the next image-specific line (M1B).
