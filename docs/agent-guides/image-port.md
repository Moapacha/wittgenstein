...
| 10  | Streaming + parallel-compile stub seams: `ImageCodec.decode` accepts an optional `onChunk` callback shape in its signature (no-op at M1A). The seam lives in `packages/codec-image/src/pipeline/decoder.ts`. _(M1 idea 1, hook only — bridge lands at M1B)_                                                                                    | ✅ M1A (`PR #68`) |
